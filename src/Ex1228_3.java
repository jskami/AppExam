...
public class Ex1228_3 {
	...
	public static void main(String[] args) {
		...
		try {
			fis = new FileInputStream("reader.txt"); // AppExam폴더 내 bin(실행)폴더에 만들어 놓은 상태
			//처리 구간, 문자열을 받아내기만 하면 되는데, 문자열이 얼마나 있는지는 모르는 상태
			int i;  // 읽은 바이트 값을 저장할 변수
			while ((i = fis.read()) != -1) {  // 데이터를 읽으면 계속 진행, 못 읽었으면 -1씩 반환-> 반복문 탈출!
			...
			if (fis != null) {  // reader.txt파일이 존재하니까 파일을 오픈했다면
				try {
					fis.close();  // 닫아야지! 닫는 구문을 만들자.
				} catch (Exception ex) {
